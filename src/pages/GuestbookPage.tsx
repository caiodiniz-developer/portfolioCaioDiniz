...
export default function GuestbookPage() {
  ...
  useEffect(() => { document.title = `${gb.title.replace('.', '')} — ${SITE.name}` }, [gb.title])
  ...
  return (
    <main style={{ position: 'relative', minHeight: '100svh', background: '#0d0d0d', overflow: 'hidden' }}>
      ...
      <div style={{
        position:   'fixed',
        inset:      0,
        background: 'linear-gradient(to bottom, rgba(13,13,13,0.55) 0%, rgba(13,13,13,0.3) 40%, rgba(13,13,13,0.7) 100%)',
        zIndex:     1,
        pointerEvents: 'none',
      }} />
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(2rem,5vw,4rem) clamp(1.25rem,4vw,2.5rem) 6rem' }}>
          ...
          {!CONFIGURED ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
              style={{ padding: '1.5rem', borderRadius: 14, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(12px)' }}>
...
CREATE POLICY "delete_all" ON guestbook FOR DELETE USING (true);`}
            ...
            <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.18, ease: E }}
              style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem', padding: '1.5rem', borderRadius: 16, border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(13,13,13,0.55)', backdropFilter: 'blur(16px)' }}>
              ...
              {entries.map((entry, i) => {
                ...
                return (
                  <motion.div
                    ...
                    style={{ display: 'flex', gap: '0.9rem', padding: '1rem 1.2rem', borderRadius: 14, border: `1px solid ${isAdmin ? 'rgba(248,113,113,0.12)' : 'rgba(255,255,255,0.07)'}`, background: 'rgba(13,13,13,0.5)', backdropFilter: 'blur(12px)', transition: 'border-color 0.2s', alignItems: 'flex-start' }}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '0.35rem' }}>
                        ...
                        <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.22)', letterSpacing: '0.04em' }}>{timeAgo(entry.created_at, lang)}</span>
                      </div>
